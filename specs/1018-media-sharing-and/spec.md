...
- **Metadata minimization scope** (FR-014/SC-008): the guarantee applies to all RE-ENCODED media and every thumbnail/poster (canvas-generated or transcoded → metadata-free). When the user explicitly picks **'Original'** quality, the source file is sent byte-for-byte and may still carry its embedded capture metadata (e.g. EXIF GPS); stripping metadata in-place without re-encoding the original is tracked as a follow-up. All of this stays inside the sealed payload — the server never receives it in the clear regardless.
