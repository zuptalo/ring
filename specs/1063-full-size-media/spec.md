...
4. **Given** a very small image (native smaller than the max width), **When** it renders, **Then** it is not upscaled beyond its native size.
...
- **FR-004**: Media MUST NOT be upscaled beyond its native resolution.
...
- **SC-004**: Small media is not upscaled past native size.
