...
3. On any message, tap the **reaction button** in the bottom row → a popover shows 5
...
4. Tap an emoji → applied, popover closes. Re-open, tap "+", pick a new emoji →
   applied; after a few uses it shows among the 5. (SC-003)
5. **Tap** a text message (or a media bubble's empty/footer area) → the full menu opens
   (reply/forward/edit/save/copy/select/delete/info/reactions/view). (SC-004)
...
- **5 + "+" visible**: open the reaction button popover; assert 5 emoji buttons and the
...
- **custom enters most-used**: react with a non-default emoji via the existing tally;
  assert `__ringTest.quickReactEmojis(5)[0]` becomes it. (SC-003)
...
- Menu in e2e: a single `click()` on a text bubble opens the menu (no long-press);
  a `click()` on a media element opens the viewer.
