...
- [ ] T004 Resolve the **D3 sealing decision** (CHK007/CHK028): pin the exact primitive/key for sealing the `{kind,state}` payload (sealed-box to the peer's identity key vs. a derived per-contact key) by inspecting how existing sealed control payloads (call-key / stream-id) are keyed; get security-review sign-off; update `research.md` D3 and tick the checklist. **Blocks T007.** Invariant: server sees no plaintext kind, no new primitive, no Double-Ratchet advance.
