...
- [~] T009 Client: wire dispatch in `src/composables/useSync.ts` — **DONE: `clearTyping()` wired into the offline branch + logout** alongside `clearPresence()`. **PENDING (needs T007 sealing):** extend the `live` fast-path predicate so `activity` frames bypass the serialized `inboundChain` and are unsealed → `applyActivity`; add a `sendActivity()` emit helper on the transient `sendLive()`. Depends on T006, T007, T008.
