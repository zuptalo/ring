...
| IV. Crypto Discipline | ✅ PASS (light touch) | Reuses the existing libsodium sealing used for opaque peer-to-peer control payloads to seal the tiny activity kind; **no new primitive, no ratchet scheme**. Mechanism chosen to avoid Double-Ratchet churn (see research.md). Security review via the required checklist. `messaging.ts` stays crypto-only and is untouched. |
