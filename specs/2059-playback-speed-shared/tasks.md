...
- [ ] T002 Run `npx vitest run` and record it passing (the new registry tests join this suite)

## Phase 2: Test seam (Foundational — no behavior change)

- [ ] T003 Add `sendVoice(chatId, name?, durationSec?)` to `src/services/testhook.ts`, mirroring the
      ...
      cannot be satisfied
- [ ] T004 Add `postVoice(text?, durationSec?)` to `src/services/testhook.ts` for a **Wall voice
...
- [ ] T005 Add `audioElementRate()` to `src/services/testhook.ts`, returning the shared audio
...
- [ ] T006 Run `npm run build` — the hooks compile, no behavior changed, the bug still reproduces

## Phase 3: RED — the regression tests (Constitution III gate) 🚨

- [ ] T007 Write `src/composables/usePlaybackRates.test.ts` against the not-yet-existing registry:
...
- [ ] T008 [US1] Create `e2e/playback-speed.spec.ts`: send two voice messages, change the speed of
...
- [ ] T009 [US1] Add the "it actually plays at that speed" leg via the `audioElementRate()` hook
...
- [ ] T010 [US2] Add the video case: set a non-default speed in the viewer, swipe to another item
...
- [ ] T011 [US3] Add the Wall case: two voice posts in the feed, change one, assert the other still
...
- [ ] T012 [US1] Add the remaining US1 acceptance legs: a voice message set to 2× still reads 2×
      when you come back to it later (AC-3), and a shared-audio card and a voice message do not
      affect each other (AC-4)
- [ ] T013 **Run the vitest and the e2e and observe them FAIL**, for the right reasons — the unit
...
- [ ] T014 Add `src/composables/usePlaybackRates.ts`: a module-level reactive
...
- [ ] T015 [P] Add `src/composables/usePlaybackRates.ts` to `coverage.include` in `vitest.config.ts`
...
- [ ] T016 [US1] In `src/composables/useAudioPlayer.ts`: `playAudio` applies the track's OWN rate
...
- [ ] T017 [US1] `src/components/VoicePlayer.vue`: `rate` reads `rateFor(props.mid)` (~:73) and
      `cycleRate()` cycles `props.mid` (~:117)
- [ ] T018 [US1] `src/views/detail/ChatDetailPage.vue`: the AudioCard binding (~:337, ~:341) reads
...
- [ ] T019 [US1] Run the e2e — the US1 pill and playback-rate cases pass

## Phase 6: User Story 2 — video keeps its speed (Priority: P1)

- [ ] T020 [US2] `src/components/VideoPlayer.vue`: accept an item id prop; replace the local
...
- [ ] T021 [US2] `src/components/MediaViewer.vue`: pass the item id to `<video-player>` (~:71-81),
      alongside the `:start-at="positions[it.id]"` it already passes for scrub position
- [ ] T022 [US2] Run the e2e — the swipe-away-and-back and close-and-reopen cases pass

## Phase 7: User Story 3 — Wall voice posts (Priority: P2)

- [ ] T023 [US3] Verify only — the Wall's two `<voice-player>` usages already pass a stable
...
- [ ] T024 [US3] Run the e2e — the Wall case passes
...
- [ ] T026 [P] Run `npm run test:unit:coverage` (not bare `vitest run` — the thresholds only apply
      under `--coverage`, so T015's ratchet entry gates nothing otherwise) and `npm run build`
- [ ] T027 [P] Run `npx playwright test e2e/media-viewer.spec.ts e2e/wall.spec.ts
      e2e/album-posts.spec.ts` — the viewer and the Wall players are unregressed
- [ ] T028 Flip `**Status**:` to `in-progress` (then `in-review`) in the spec and run `make roadmap`
- [ ] T029 **Bump `package.json` to 1.0.33** — `develop` is at 1.0.32 with tag `v1.0.32` cut, so
