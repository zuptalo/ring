...
- [ ] T001 Run `npm run build` and record it passing on the untouched branch
- [ ] T002 Run `npx playwright test e2e/seen-on-view.spec.ts` and record it passing — this is the
...
- [ ] T003 Add optional `dlFailedAt?: number` to the `Message` interface in `src/db/types.ts` (near
...
- [ ] T004 Add `seedPendingIncoming(chatId, kind, opts?)` to `src/services/testhook.ts` that writes
...
- [ ] T005 [P] Extend the `messages(chatId)` projection in `src/services/testhook.ts` (~:551-581) to
      ...
      second call per row)
- [ ] T006 **RED (unit)**: write the vitest for the auto-retry bound *before* the function exists —
...
- [ ] T007 Extract the auto-retry bound as a **pure** function in `src/utils/` (or beside the
...
- [ ] T008 Run `npm run build` — types compile, behavior unchanged, bug still reproduces by hand
...
- [ ] T009 Create `e2e/voice-pending.spec.ts` with the US1 case: seed a **fetchable** pending
...
- [ ] T010 [US2] Add the US2 case: with a `broken: true` seed, the bubble shows a placeholder
      carrying the duration, a tap starts a retry, and the failure is surfaced
- [ ] T011 [US2] Add the FR-008 persistence case: after a failed fetch, **leave the chat and
...
- [ ] T012 Add the FR-013 / SC-004 case: seed a pending incoming voice message with an old
      `updatedAt` (a message stranded before this fix), open the chat, assert it recovers with no
      tap and no re-send
- [ ] T013 **Run the new spec and observe it FAIL.** Record the failure output in the PR/commit
...
- [ ] T014 [US1] Add the pending-voice branch to the `v-if` chain in
...
- [ ] T015 [US1] Add `.vp-pending` CSS to the same file's `<style>` block beside the existing
...
- [ ] T016 [US1] Add `downloadLane = createLimiter(3)` in `src/db/queries.ts` beside the existing
...
- [ ] T017 [US1] Add a debounced `recoverVisiblePending()` in `src/views/detail/ChatDetailPage.vue`
...
- [ ] T018 [US1] Bound the automatic path with the pure `shouldAutoRetry` from T007, backed by a
...
- [ ] T019 [US1] Run `npx playwright test e2e/voice-pending.spec.ts` — the US1 and FR-013 cases pass
...
- [ ] T020 [US2] In `src/db/queries.ts` `downloadMessageMedia`, stamp `dlFailedAt = now()` on failure
...
- [ ] T021 [US2] Give the failed state a face in `src/views/detail/ChatDetailPage.vue`: when
      `m.dlFailedAt` is set the placeholder reads as failed and offers a retry, rather than looking
      like it is still loading
- [ ] T022 [US2] Change `downloadPendingMedia` (~:1441) to
...
- [ ] T023 [US2] Confirm the bubble repaints on the failed flip: T020 bumps `updatedAt`, which is
...
- [ ] T024 [US2] Add the SC-003 recovery case to `e2e/voice-pending.spec.ts`: seed a **fetchable**
      ...
      exercised
- [ ] T025 [US2] Run `npx playwright test e2e/voice-pending.spec.ts` — the US2, FR-008 persistence
...
- [ ] T026 [US3] Add a **dedicated sibling branch** for pending round notes in
...
- [ ] T027 [P] [US3] Add the round-note cases to `e2e/voice-pending.spec.ts` via
...
- [ ] T028 [US4] Replace the swallowing `catch` in `downloadPendingMedia`
...
- [ ] T029 [US4] Show the failed state on the existing pending photo/video block and audio/file chip
      so those kinds read as failed too (FR-010)
- [ ] T030 [P] [US4] Add the US4 case to `e2e/voice-pending.spec.ts`: a pending **photo** whose tap
      fails produces a visible message and a failed-looking bubble
- [ ] T031 [P] [US4] Add the FR-015 guard case: with photo auto-download set to never, a pending
...
- [ ] T032 Add the SC-005 stampede case to `e2e/voice-pending.spec.ts`: seed 10 consecutive pending
      voice messages, scroll through them, assert all 10 recover, **at most 3 fetches in flight**,
      and the scroll position stays stable
- [ ] T033 Add the SC-001 matrix case: walk the six kinds × four states and assert **no** bubble
...
- [ ] T034 Run `npx playwright test e2e/seen-on-view.spec.ts` — the read-receipt observer is
      unharmed by T017 (the canary from T002)
- [ ] T035 [P] Run `npx playwright test e2e/media-blob-delete.spec.ts e2e/chat-media-scroll.spec.ts
      e2e/media-cleanup.spec.ts` — pending/download and scroll-anchor behavior unregressed
- [ ] T036 [P] Run `npx vitest run` and `npm run build` — unit (now including T006) + typecheck green
...
- [ ] T038 Flip `**Status**:` to `in-progress` (then `in-review`) in
      `specs/2058-voice-messages-arrive/spec.md` and run `make roadmap`
- [ ] T039 **Bump `package.json` to 1.0.33.** `develop` and `main` are level at 1.0.32 (tag
