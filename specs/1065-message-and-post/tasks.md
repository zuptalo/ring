...
- [ ] T050 [US4] Extend `classifyPostActivity` in `src/services/sw-inbox.ts` (~line 1774) to open comment payloads and read the sealed parent, so a closed app can say "replied to you"; on a missing post key or a failed decrypt, fall through to the generic rather than guessing, and stay inside the wake deadline (research §R9, §R12, checklist CHK038, CHK041)
